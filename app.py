...
SISTEM_MESAJI = """Sen dünyanın en iyi oyun programlama asistanısın. Görevin: Kullanıcının istediği oyunu EN AZ PARÇAYA BÖLEREK, en uzun ve eksiksiz kod bloklarıyla yazmak. Eğer kod çok uzunsa ve kesilirse, bir sonraki mesajda tam olarak kaldığın yerden (en son karakterden) hiçbir şeyi tekrar etmeden devam etmelisin. Kodların modüler, profesyonel and hatasız olmalı. Sadece kod and gerekli açıklamaları ver."""
...
with tab2:
    ...
    if st.button("Kodu Güncelle"):
        ...
        if eski_kod and degisiklik:
            ...
            # DÜZELTİLEN KISIM: f-string tek bir satırda, kaçış karakterleri (\\n) kullanılarak güvenli hale getirildi.

            istek_metni = f"Mevcut Kod:\n```python\n{eski_kod}\n```\n\nBu kod üzerinde şu değişikliği yap: {degisiklik}"
            ...
            st.warning("Lütfen hem mevcut kodu hem de yapılacak değişikliği doldurun.")
